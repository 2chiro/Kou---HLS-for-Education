...
export default class Canvas extends Component {
    ...
    renderCanvas(ctx) {
        ...
        //マルチサイクル演算対応
        const pt = { 'A': nodeInfo.ptAdd, 'S': nodeInfo.ptSub, 'M': nodeInfo.ptMult, 'D': nodeInfo.ptDiv, 'I': 1, 'O': 1}
            ...
            if (this.props.id < 4) {
                ...
                if (this.state.selectEdge) {
                    ...
                    switch (this.state.selectEdgeStore[1]) {
                        case 0:
                            ctx.lineTo((nodeX[this.state.selectEdgeStore[0]] - this.state.origin_x) * ratio, (nodeY[this.state.selectEdgeStore[0]] - this.state.origin_y + 35 + (50 * (pt[nodeType[this.state.selectEdgeStore[0]]] - 1))) * ratio)
                            break
                        case 1:
                            ctx.lineTo((nodeX[this.state.selectEdgeStore[0]] - this.state.origin_x - 20) * ratio, (nodeY[this.state.selectEdgeStore[0]] - this.state.origin_y - 35 - (50 * (pt[nodeType[this.state.selectEdgeStore[0]]] - 1))) * ratio)
                            break
                        case 2:
                            ctx.lineTo((nodeX[this.state.selectEdgeStore[0]] - this.state.origin_x + 20) * ratio, (nodeY[this.state.selectEdgeStore[0]] - this.state.origin_y - 35 - (50 * (pt[nodeType[this.state.selectEdgeStore[0]]] - 1))) * ratio)
                            break
                ...
                for (var i in nodeEdge1) {
                    ...
                    ctx.beginPath()
                    // マルチサイクル演算対応
                    ctx.moveTo((nodeX[nodeEdge1[i]] - this.state.origin_x) * ratio, (nodeY[nodeEdge1[i]] - this.state.origin_y + 35 + (50 * (pt[nodeType[nodeEdge1[i]]] - 1))) * ratio)
                    switch (nodeEdgeType[i]) {
                        case 'l':
                            ctx.lineTo((nodeX[nodeEdge2[i]] - this.state.origin_x - 20) * ratio, (nodeY[nodeEdge2[i]] - this.state.origin_y - 35 - (50 * (pt[nodeType[nodeEdge2[i]]] - 1))) * ratio)
                            ctx.font = font
                        ...
                        case 'r':
                            ...
                            break
                        case 'c':
                            ctx.lineTo((nodeX[nodeEdge2[i]] - this.state.origin_x) * ratio, (nodeY[nodeEdge2[i]] - this.state.origin_y - 35 - (50 * (pt[nodeType[nodeEdge2[i]]] - 1))) * ratio)
                            ctx.font = font
                            ctx.fillText(i, ((nodeX[nodeEdge1[i]] + nodeX[nodeEdge2[i]]) / 2 - this.state.origin_x + 20) * ratio,
                                ((nodeY[nodeEdge1[i]] + nodeY[nodeEdge2[i]]) / 2 - this.state.origin_y) * ratio)
                            break
                    ...
                    switch (nodeType[i]) {
                        ...
                        case 'D':
                            if (targetALUNode.indexOf(Number(i)) > -1) {
                                drawOpe(ctx, (nodeX[i] - this.state.origin_x) * ratio, (nodeY[i] - this.state.origin_y) * ratio, ratio, nodeType[i], pt[nodeType[i]], 1, this.props.id)
                            } else if (this.state.selectNode[0] == i && this.props.dfgMode === 9) {
                                drawOpe(ctx, (nodeX[i] - this.state.origin_x) * ratio, (nodeY[i] - this.state.origin_y) * ratio, ratio, nodeType[i], pt[nodeType[i]], 0, this.props.id, this.state.selectNode[1])
                            } else {
                                drawOpe(ctx, (nodeX[i] - this.state.origin_x) * ratio, (nodeY[i] - this.state.origin_y) * ratio, ratio, nodeType[i], pt[nodeType[i]], 0, this.props.id)
                            }
                            break
                        case 'I':
                    ...
                    switch (this.props.id) {
                        case 1:
                            switch (this.props.dfgMode) {
                                case 3:
                                    drawOpe(ctx, this.state.mouse_x, this.state.mouse_y, ratio, 'A', pt['A'])
                                    break
                                case 4:
                                    drawOpe(ctx, this.state.mouse_x, this.state.mouse_y, ratio, 'S', pt['S'])
                                    break
                                case 5:
                                    drawOpe(ctx, this.state.mouse_x, this.state.mouse_y, ratio, 'M', pt['M'])
                                    break
                                case 6:
                                    drawOpe(ctx, this.state.mouse_x, this.state.mouse_y, ratio, 'D', pt['D'])
                                    break
                                case 7:
                ...
                function drawOpe(ctx, x, y, ratio, type, pt, i, id, j) {
                    //下部接続線
                    ctx.strokeStyle = "rgb(20, 20, 20)"
                    ctx.lineWidth = "2px"
                    ctx.beginPath()
                    ctx.moveTo(x, y)
                    ctx.lineTo(x, y + ((35 + (50 * (pt - 1))) * ratio))
                    ctx.closePath()
                    ctx.stroke()
                    ctx.fillStyle = "rgb(200, 100, 40)"
                    ctx.beginPath()
                    if (j == 0) {
                        ctx.arc(x, y + ((35 + (50 * (pt - 1))) * ratio), (5 + (4 * connectTime / 10)) * ratio, 0, Math.PI * 2, false)
                        ctx.fill()
                        ctx.stroke()
                    } else {
                        ctx.arc(x, y + ((35 + (50 * (pt - 1))) * ratio), 5 * ratio, 0, Math.PI * 2, false)
                        ctx.fill()
                        ctx.stroke()
                    }
                    //上部接続線
                    ...
                    ctx.beginPath()
                    ctx.moveTo(x, y - (50 * (pt - 1) * ratio))
                    ctx.lineTo(x + (20 * ratio), y - ((35 + (50 * (pt - 1))) * ratio))
                    ctx.closePath()
                    ctx.stroke()
                    ctx.beginPath()
                    ctx.moveTo(x, y - (50 * (pt - 1) * ratio))
                    ...
                    ctx.beginPath()
                    if (j == 2) {
                        ctx.arc(x + (20 * ratio), y - ((35 + (50 * (pt - 1))) * ratio), (5 + (4 * connectTime / 10)) * ratio, 0, Math.PI * 2, false)
                        ctx.fill()
                        ctx.stroke()
                    } else {
                        ctx.arc(x + (20 * ratio), y - ((35 + (50 * (pt - 1))) * ratio), 5 * ratio, 0, Math.PI * 2, false)
                        ctx.fill()
                        ctx.stroke()
                    }
                    ctx.beginPath()
                    ...
                    ctx.beginPath()
                    //マルチサイクル演算対応
                    ctx.arc(x, y - (50 * (pt - 1) * ratio), 20 * ratio, Math.PI, 0, false)
                    ctx.lineTo(x + (20 * ratio), y + (50 * (pt - 1) * ratio))
                    ctx.arc(x, y + (50 * (pt - 1) * ratio), 20 * ratio, 0, Math.PI, false)
                    ctx.lineTo(x + (-20 * ratio), y - (50 * (pt - 1) * ratio))
                    ctx.fill()
                    ctx.stroke()
        ...
        function selectNode(x, y, nodeInfo) {
            ...
            for (var i in nodeType) {
                ...
                switch (nodeType[i]) {
                    ...
                    case 'D':
                        // ノード部
                        if (Math.pow(nodeX[i] - x, 2) + Math.pow(nodeY[i] - y + (50 * (pt[nodeType[i]] - 1)), 2) <= Math.pow(20, 2)) {
                            selectnode.splice(0, 1, i)
                            return selectnode
                        }
                        if (Math.pow(nodeX[i] - x, 2) + Math.pow(nodeY[i] - y - (50 * (pt[nodeType[i]] - 1)), 2) <= Math.pow(20, 2)) {
                        ...
                        if (nodeX[i] - 20 < x && nodeX[i] + 20 > x) {
                            if (nodeY[i] - (50 * (pt[nodeType[i]] - 1)) < y && nodeY[i] + (50 * (pt[nodeType[i]] - 1)) > y) {
                                selectnode.splice(0, 1, i)
                                return selectnode
                            }
                        }
                        ...
                        // 接続部
                        if (Math.pow(nodeX[i] - x, 2) + Math.pow(nodeY[i] - y + 35 + (50 * (pt[nodeType[i]] - 1)), 2) <= Math.pow(9, 2)) {
                            selectnode.splice(0, 2, i, 0)
                            return selectnode
                        }
                        if (Math.pow(nodeX[i] - x - 20, 2) + Math.pow(nodeY[i] - y - 35 - (50 * (pt[nodeType[i]] - 1)), 2) <= Math.pow(9, 2)) {
